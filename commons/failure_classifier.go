...
// classifyCommandFailure maps a failed install command's output to a
// plain-language cause and a short "Try:" remedy, so the console node page
// shows the customer something actionable instead of a raw stderr dump.
//
...
// never returns an empty cause: when nothing matches, or output is empty, it
// falls back to a generic, step-named message that points at the full log.
func classifyCommandFailure(step, command, output string) (cause string, remedy string) {
	...
	case contains(
		...
		return "Another package manager (apt, dpkg, or unattended-upgrades) is holding the system package lock, so the install could not proceed.",
	...
	case contains("no space left on device", "write error: no space"):
		return "The node ran out of disk space during installation.",
	...
	case contains(
		...
		return "The node could not reach the network to download packages or contact RunOS (DNS resolution or an outbound connection failed).",
	...
	case contains(
		...
		return "A required package or package version is not available from the node's configured apt repositories.",
	...
	case contains(
		...
		return "The install was blocked by held, broken, or conflicting packages already on the node.",
	...
	case contains("permission denied", "operation not permitted", "must be run as root", "are you root"):
		return "The install command did not have the privileges it needed (permission denied / not running as root).",
	...
	case contains(
		...
		return "Kubernetes setup (kubeadm) failed during a preflight check or an init/join phase, so the node did not join the cluster.",
	...
	case contains(
		...
		return "The container runtime (containerd/CRI) could not pull an image or was not reachable, so workloads could not start.",
	...
	case contains(
		...
		return "An apt repository is misconfigured or its signing key is missing, so package metadata could not be trusted.",
	...
	// --- generic fallback -----------------------------------------------
	default:
		return genericCause(step), "review the full command output in /var/log/runos.log on the node to see why it failed, fix the underlying issue, then retry the install."
	}
}
