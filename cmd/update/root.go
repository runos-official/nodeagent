...
With no flags, updates to the version advertised by the installer. Pass an exact
release tag with --version to pin to a specific build (the installer is
fail-closed and never falls back to a floating "latest"). Use --json to emit a
single machine-readable result object instead of the human-readable summary.`,
